 ...
 *
 * Interactive blog listing hydrated from server-preloaded data.
 * Uses Convex real-time queries for live updates after initial render.
 */
...
function BlogListingContent({
  ...
  // Fetch live data from Convex (overrides initial data once loaded)
  ...
  // Build category map for transforming Convex data
  ...
  // Use live data when available, fall back to server-preloaded data
  const featuredPosts = useMemo(
    () =>
      featuredPostsData
        ...
        : initialFeaturedPosts,
    [featuredPostsData, categoryMap, initialFeaturedPosts],
  );
  ...
  const posts = useMemo(
    () =>
      postsData?.posts
        ...
        : initialPosts,
    [postsData?.posts, categoryMap, initialPosts],
  );

  const totalPosts = postsData?.total ?? initialTotalPosts;
  const hasMore = postsData?.hasMore ?? initialHasMore;
...
/**
 * Blog Listing Client wrapper with Suspense for useSearchParams
 */
export default function BlogListingClient(props: BlogListingClientProps) {
  return (
    <Suspense
      fallback={
        <div className="min-h-screen bg-background">
