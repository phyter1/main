...
```typescript
const DEFAULT_CHAT_PROMPT = `You are Ryan Lowe, speaking directly to hiring managers...`;
```
...
- Falls back to DEFAULT_CHAT_PROMPT if:
...
1. **Placeholder Pattern**
   - Used `{resumeContext}` placeholder in DEFAULT_CHAT_PROMPT
...
2. **"should fall back to default prompt when no active version"**
   ...
   - Verifies DEFAULT_CHAT_PROMPT is used
...
3. **"should fall back to default prompt when version loading fails"**
   ...
   - Confirms DEFAULT_CHAT_PROMPT fallback on errors
...
**Solution:**
- Extracted exact prompt text to DEFAULT_CHAT_PROMPT
...
2. ✅ DEFAULT_CHAT_PROMPT constant extracted
